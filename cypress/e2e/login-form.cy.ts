describe('Testing login form', () => {
    beforeEach(() => {
        cy.visit('localhost:3001/auth/login', { timeout: 10000 });
    });
    ...
    it('Render login page', () => {
        ...
        cy.get('[data-cy="loginButton"]').contains('Login').should('be.visible');
        cy.get('[data-cy="continueWithGoogleButton"]').contains('Continue with Google').should('be.visible');
        cy.get('[data-cy="signUpLink"]').contains('Sign up').should('be.visible');
        cy.get('[data-cy="forgotPasswordLink"]').contains('Forgot your password?').should('be.visible');
    });
