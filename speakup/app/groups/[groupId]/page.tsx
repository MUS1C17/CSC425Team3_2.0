      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          <div className="space-y-2 lg:col-span-2">
            ...
            <p className="text-xs text-muted-foreground">Created {new Date(group.created_at).toLocaleString()}</p>
          </div>

          {isOwner ? (
            <div className="flex flex-col items-start gap-3 lg:col-span-1 lg:justify-self-end">
              <Button asChild>
        ...
        <Card className="bg-card/60 border border-white/10 md:-rotate-1">
          <CardHeader className="space-y-1">
            ...
            <CardDescription>Jump into a session to participate live or catch up later.</CardDescription>
          </CardHeader>
          <CardContent>
            {!filteredSessions.length ? (
              <div className="rounded-md border border-dashed px-4 py-6 text-center text-sm text-muted-foreground">
                {isOwner
                  ? "No sessions yet — create your first and start collecting questions."
