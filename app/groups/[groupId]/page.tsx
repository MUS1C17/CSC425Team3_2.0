...
import type { Group, Session } from "@/lib/database.types";
...
export default async function GroupPage({ params }: GroupPageProps) {
  ...
  const [groupResp, membershipResp, sessionsResp] = await Promise.all([
    supabase
      .from("groups")
  ...
  if (groupResp.error) throw new Error(groupResp.error.message);
  if (sessionsResp.error) throw new Error(sessionsResp.error.message);

  const group = (groupResp.data ?? null) as any;
  ...
  const sessionMembershipMap = new Map<string, string>();
  (mySessionMemberships ?? []).forEach((row: any) => {
    sessionMembershipMap.set(row.session_id, row.role ?? "");
  });

  const role = (membershipResp.data as { role?: string } | null)?.role ?? null;
