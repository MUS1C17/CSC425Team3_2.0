...
export default async function GroupsIndexPage() {
  ...
  const membershipResp = await supabase
    ...
    .eq("user_id", user.id);
  const membershipRows = (membershipResp.data ?? []) as any[];
  const membershipError = membershipResp.error;
  ...
  const groupsResp = groupIds.length
    ? await supabase
        .from("groups")
  ...
  const groupsError = groupsResp.error;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create a new class, or jump into an existing one with your join code.
          </p>
        </div>
